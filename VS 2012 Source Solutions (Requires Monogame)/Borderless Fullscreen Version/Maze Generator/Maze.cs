...
namespace Maze_Generator
{
    ...
    class Maze
    {
        // Enumerated values representing the various generation states for the Maze.
        ...
        public Maze(Texture2D texture, Vector2 positionOnWindow, uint coordinateSize, uint nodeDrawSize, uint wallDrawThickness)
        {
            ...
            // Initialises the lists containing the objects which make up the Maze.
        ...
        // Sets the Maze to begin being generated.
        ...
        {
            // Sets the generation origin coordinate to the parameter if a valid coordinate within the Maze, else sets it to (0, 0).
            ...
            // Fills the Maze with the specified amount of nodes and walls connecting each of them.
            ...
            while (generationState == GenerationState.BeingGenerated && generationCounter < generationSpeed)
            {
                if (generationState == GenerationState.BeingGenerated)
                {
                    ...
                    while (true)
                    {
                        ...
                        List<MazeNode> unvisitedAdjacentNodes = currentNode.GetUnvisitedAdjacentNodes(this);
                        ...
                        if (unvisitedAdjacentNodes.Count == 0)
                        {
                            ...
                            while (nodesQueue.Count > 0)
                            {
                                ...
                                MazeNode tempNode = nodesQueue.Dequeue();
                    ...
                    if (generationState == GenerationState.BeingGenerated)
                    {
                        ...
                        List<MazeNode> adjacentNodes = currentNode.GetUnvisitedAdjacentNodes(this);
                        ...
                        {
                            if (walls[i].FirstNodeCoordinate == currentNode.CoordinateInMaze && walls[i].SecondNodeCoordinate == adjacentNode.CoordinateInMaze ||
                                walls[i].FirstNodeCoordinate == adjacentNode.CoordinateInMaze && walls[i].SecondNodeCoordinate == currentNode.CoordinateInMaze)
                            {
                                walls.RemoveAt(i);

                                adjacentNode.WalkDistance = currentNode.WalkDistance + 1;
        ...
        // The current Maze node to be used in the Maze generation.
        private MazeNode currentNode;
        // The queue used to hold the Maze nodes that are still to be visited.
        private Queue<MazeNode> nodesQueue;
        ...
        // The seed used by the random number generator to generate the Maze, set to 0 if not to be used.
        ...
        // The length in coordinates of the longest walk distance of any path originating at the generation node within the Maze.
        private uint longestWalkDistance;
        // The number of walls that are removed during every generation update.
