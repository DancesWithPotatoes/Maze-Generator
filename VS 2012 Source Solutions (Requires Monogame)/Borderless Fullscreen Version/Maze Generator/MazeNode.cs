...
namespace Maze_Generator
{
    ...
    {     
        ...
        // Draws the MazeNode on the game window using the specified texture.
        public void Draw(Color color, Maze maze, SpriteBatch spriteBatch, Texture2D texture)
        {
            spriteBatch.Draw(texture, new Rectangle((int)(maze.PositionOnWindow.X + coordinateInMaze.X * maze.CoordinateSize + maze.CoordinateSize / 2 + maze.WallDrawThickness / 2 - maze.NodeDrawSize / 2),
                (int)(maze.PositionOnWindow.Y + coordinateInMaze.Y * maze.CoordinateSize + maze.CoordinateSize / 2 + maze.WallDrawThickness / 2 - maze.NodeDrawSize / 2), (int)maze.NodeDrawSize,
                (int)maze.NodeDrawSize), color);
        }
        ...
        // Whether or not the MazeNode has been visited during the maze generation process.
        private bool visited;
        // The distance in coordinates which must be traveled (accounting for walls) from a starting node in the maze to this node.
